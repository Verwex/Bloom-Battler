...
function makeLoot(message, prefix) {
    ...
    if (incorrectItems.length > 0)
        message.channel.send(`⚠️ **Your invalid items are:**${incorrectItems}`);
    ...
    if (chanceInput.length > itemInput.length) {
        ...
        message.channel.send("⚠️ Due to there being more drop chances than items themselves, I'll shorten them for you.");
    }

    if (chanceInput.length < itemInput.length) {
        message.channel.send("⚠️ Due to there being more items than drops chances themselves, I'll assign the leftover ones a default of 10%.");
