	...
	if (skillDefs.type === "heal") {		
		...
		if (counterSkill) {
			...
			if (dmg[1] !== "miss" && dmg[1] !== "block" && dmg[1] !== "drain") {
				if (dmg[2] == true) {
					finaltext = finaltext + `<:effective:876899270731628584>${dmg[6] != 1.5 && dmg[6] != null ?  `(${dmg[6]}x)` : ``}`;
				} else if (dmgtype === "resist") {
					finaltext = finaltext + `<:resist:877132670784647238>${dmg[6] != 0.5 && dmg[6] != null ?  `(${dmg[6]}x)` : ``}`;;
				}
